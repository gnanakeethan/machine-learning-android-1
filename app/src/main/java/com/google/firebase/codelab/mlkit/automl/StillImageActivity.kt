...
// Setup our BluetoothManager
    ...
    val pairedDevices = bluetoothManager.getPairedDevicesList();
  ...
  private fun classifyImage(bitmap: Bitmap) {
    ...
    classifier?.classifyFrame(bitmap)?.addOnCompleteListener { task ->
      if (task.isSuccessful) {
        ...
        Log.e(TAG,"Result" +task);
        //TODO: Parse and send a proper message from task result
  ...
  private fun onConnected(connectedDevice: BluetoothSerialDevice) {
    ...
    deviceInterface!!.setListeners(
      { this.onMessageReceived(it) },
      { this.onMessageSent(it) },
      { this.onError(it) })
